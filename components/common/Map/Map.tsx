...
import {
  ...
  districtList,
  provinceList,
} from "@/public/assets/data/intData";
import { CommonGlobal } from "@/types/common";

interface IProps {
  handlePostAddress(data: any): void;
  handleCloseModalMap(): void;
  initLocation?: CommonGlobal.AddressUser;
  handleChangeMainAddress?: (data: any) => void;
}
  ...
  const onPressSearch = async () => {
    ...
    if (txtSearch.trim()) {
      const url = `https://rsapi.goong.io/Place/AutoComplete?api_key=${
        process.env.NEXT_PUBLIC_GOONG_KEY_API
      }&input=${txtSearch.trim()}`;
      try {
    ...
    if (resData?.results?.[0]) {
      ...
      const provinceId =
        provinceList.find(
          (i) =>
            i.name.lastIndexOf(
              addressDetail.address_components?.[index]?.short_name,
            ) >= 0,
        )?.id || 0;
      const districtId =
        districtList.find(
          (i) =>
            ...
            i.name.lastIndexOf(
              addressDetail.address_components?.[index - 1]?.short_name,
            ) >= 0,
        )?.id || 0;
      const communeId =
        communeList.find(
          (i) =>
            ...
            i.name.lastIndexOf(
              addressDetail.address_components?.[index - 2]?.short_name,
            ) >= 0,
        )?.id || 0;
  ...
  return (
    <div className={`map ${styles.map}`}>
      <div className={styles.map_header}>
        ...
        />
        <button
          className={styles.map_header_btn}
          type="button"
      ...
      <div className={styles.map_main_list}>
        {loading ? (
          ...
          listAddress.map((item, idx) => (
            <div
              key={idx}
              className={styles.map_main_item}
